"""The :mod:`async_pykka.messages` module contains actor messages.
...
without touching async_pykka's internals.

An example is to combine :meth:`~async_pykka.ActorRef.ask` and :class:`ProxyCall`
to call a method on an actor without having to spend any resources on creating
a proxy object::

    reply = await actor_ref.ask(
        ProxyCall(
            attr_path=['my_method'],
            args=['foo'],
            kwargs={'bar': 'baz'}
...
Another example is to use :meth:`~async_pykka.ActorRef.tell` instead of
:meth:`~async_pykka.ActorRef.ask` for the proxy method call, and thus avoid the
